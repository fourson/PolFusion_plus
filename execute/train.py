...
def train(config, resume, **extra_args):
    ...
    # valid_data_loader = data_loader.split_validation()  # automatically configure valid_data_loader
    valid_data_loader_class = getattr(module_data, config['valid_data_loader']['type'])  # manually configure
    valid_data_loader = valid_data_loader_class(**config['valid_data_loader']['args'])
    ...
    metrics = dict()
    for variable_name, mets in config['metrics'].items():
        metrics[variable_name] = [getattr(module_metric, met) for met in mets]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PolFusion+')
