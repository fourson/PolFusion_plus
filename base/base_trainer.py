...
from model.funcs import init_weights
...
class BaseTrainer:
    ...
    def __init__(self, config, model, loss, metrics, optimizer, lr_scheduler, resume, train_logger):
        ...
        self.monitor = trainer_args.get('monitor', 'off')  # sample: "max S0 PSNR"
        ...
        if self.monitor == 'off':
            ...
            self.mnt_mode, self.mnt_variable_name, self.mnt_metric = self.monitor.split()
                ...
                if key == 'metrics':
                    for variable_name, mets in self.metrics.items():
                        for i, met in enumerate(mets):
                            log[f'{met.__name__}_{variable_name}'] = value[variable_name][i]
                elif key == 'val_metrics':
                    for variable_name, mets in self.metrics.items():
                        for i, met in enumerate(mets):
                            log[f'val_{met.__name__}_{variable_name}'] = value[variable_name][i]
                else:
                    log[key] = value
            ...
            if self.mnt_mode != 'off':
                try:
                    ...
                    improved = (self.mnt_mode == 'min' and log[
                        f'{self.mnt_metric}_{self.mnt_variable_name}'] < self.mnt_best) or \
                               (self.mnt_mode == 'max' and log[
                                   f'{self.mnt_metric}_{self.mnt_variable_name}'] > self.mnt_best)
                except KeyError:
                ...
                if improved:
                    self.mnt_best = log[f'{self.mnt_metric}_{self.mnt_variable_name}']
                    not_improved_count = 0
                    is_best = True
                else:
